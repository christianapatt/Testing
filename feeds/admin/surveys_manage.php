...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($title === "" || !is_array($questions) || count($questions) < 1) {
...
// Show latest surveys so Admin can verify answers exist.
$existingSurveys = $conn->query("
    SELECT
        s.id,
        s.title,
        s.created_at,
...
<form method="post" class="card card-body gf-survey-editor" id="surveyForm">
    <div class="gf-editor-top">
        ...
        <input
            name="title"
            class="form-control gf-form-title-input"
...
<h4 class="mt-4">Existing Surveys</h4>
<div class="table-responsive">
    <table class="table table-bordered table-striped">
        ...
        <tbody>
        <?php while ($s = $existingSurveys->fetch_assoc()): ?>
            <tr>
